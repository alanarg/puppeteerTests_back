...
const NovoOrgaoEstagio = async (req,page,i,c,ambiente) =>{
    const j = 0;
    const entradas = req;
            ...
            //Selecionando no Select pela string fornecida 
            await page.evaluate(tipo=>{
                console.log(tipo);
                let ops = document.querySelector('select#tipoOrgaos').options.length
                for (let index = 0; index < ops; index++) {
                    if(tipo===document.querySelector('select#tipoOrgaos').options[index].innerHTML){
                        document.querySelector('select#tipoOrgaos').options[index].selected = true;
                    }
...
module.exports = NovoOrgaoEstagio;